...
try:
    ...
    os.system("pip install "+ package.lower())
...
def encrypt(toEncrypt, key):
  ...
  code_method = "base64"    
  iv = ''
  cipher_text = Cipher_AES(key, iv).encrypt(toEncrypt, cipher_method, pad_method, code_method)
...
def decrypt(toDecrypt, key):
  ...
  code_method = "base64"    
  iv = ''
  text = Cipher_AES(key, iv).decrypt(toDecrypt, cipher_method, pad_method, code_method)
  return text

def deencrypt(_crypto: str, _message: str, _secretKey: str) -> str:
  if _crypto == 'encrypt':
    _return = encrypt(_message, _secretKey)
  elif _crypto == 'decrypt':
    _return = decrypt(_message, _secretKey)
  return _return

# _crypto: encrypt/decrypt 
